  ...
  <div>
    <div class="months-navigation">
      ...
      >
        ...
        <div class="value-label" v-nota-format="month.total"/>
      </div>
    </div>
    ...
    <div class="container-group">
      <div class="container">
        <div v-if="activeMonth.data && !activeMonth.data.length">
          Você não cadastrou nenhuma nota neste mês
        </div>
        <template v-else>
          <gerenciamento-notas-item
            :key="index"
            :data="item"
...
import GerenciamentoNotasItem from './GerenciamentoNotasItem'

export default {
  name: 'GerenciamentoNota',
  components: {
    GerenciamentoNotasItem
  },
  data: () => ({
    notas: [],
    activeMonth: {}
  ...
  computed: {
    groupedMonths () {
      ...
      if (this.notas.length) {
        const months = groupBy(this.notas, i => (
